...
public sealed class FaustMinion : WildHuntMinionModel
{
    ...
    {
        if (IsUpgraded) await CreatureCmd.SetMaxAndCurrentHp(self, 12);
        // 随从存活时有效——buff 挂在随从身上，随从死亡自动消失
        await PowerCmd.Apply<FaustBuffPower>(new ThrowingPlayerChoiceContext(), self, 1, owner.Creature, source);
