...
public sealed class IshmaelMinion : WildHuntMinionModel
{
    ...
    {
        if (IsUpgraded) await CreatureCmd.SetMaxAndCurrentHp(self, 16);
        // 随从存活时有效——buff 挂在随从身上，随从死亡自动消失
        await PowerCmd.Apply<IshmaelBuffPower>(new ThrowingPlayerChoiceContext(), self, 1, owner.Creature, source);
